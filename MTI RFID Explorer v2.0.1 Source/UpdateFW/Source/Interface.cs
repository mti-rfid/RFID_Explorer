...
namespace UpdateFWTool
{
    ...
    public class Interface : rfid.Linkage
    {
        ...
        public bool API_OpenDevice()
        {
            ...
            //Add by FJ for the FWUpdate failure in USB connection, 2016-09-08
            ...
            //End by FJ for the FWUpdate failure in USB connection, 2016-09-08
...
//Add by FJ for the FWUpdate failure in USB connection, 2016-09-08
//Del by FJ for the model name display error in updateFW and updateOEM tool in USB connection, 2016-06-16
        ...
        )
        {
            ...
            clsPacket.Mid_API_SetOverlapTime(200, 200);
...
//==========================Command===================================
        ...
        )
        {
            ...
            TxCmd  = ENUM_CMD.UPDATE_GET_VERSION;
            buf[0] = 0x0D; //Magic
            buf[1] = 0xF0; //Magic
        ...
        )
        {
            ...
            TxCmd  = ENUM_CMD.UPDATE_ENTER_UPDATE_MODE;
            buf[0] = 0x0D; //Magic
            buf[1] = 0xF0; //Magic
            buf[2] = (byte)( (int)TxCmd & 0xF );//command
            buf[3] = 0; //command
        ...
        )
        {
            ...
            TxCmd  = ENUM_CMD.UPDATE_COMPLETE;
            buf[0] = 0x0D; //Magic
            buf[1] = 0xF0; //Magic
            buf[2] = (byte)( (int)TxCmd & 0xF );//command;
            buf[3] = 0; //command
        ...
        public Result API_UpdateGetUpdateKey
        (
            ref uint Key
        )
        {
...
#if _TRACE_OUT_PUT
            PrintMagToTxt("API_UpdateGetUpdateKey");
#endif
            ...
            TxCmd = ENUM_CMD.UPDATE_GET_UPDATE_KEY;
            buf[0] = 0x0D; //Magic
            buf[1] = 0xF0; //Magic
            buf[2] = (byte)((int)TxCmd & 0xF);//command
            buf[3] = 0; //command
            ...
            Key_buf[0] = buf[8];
            Key = BitConverter.ToUInt32(Key_buf, 0);
...
//=============================================================
        ...
        )
        {
            ...
            if (clsPacket.Mid_API_Write(wtBuf, (UInt32)wtBuf.Length) != true)
            {
                ...
                //Write again
                return clsPacket.Mid_API_Write(wtBuf, (UInt32)wtBuf.Length);
                //return clsPacket.TRANS_API_Write(wtBuf, (UInt32)wtBuf.Length);
