...
namespace rfid
{
    ...
    public partial class Linkage
    {
        ...
        )
        {
            ...
            bool bGetUSB = false;
            //Add by FJ for the model name display error in updateFW and updateOEM tool in USB connection, 2016-06-16
            clsPacket.TRANS_API_Close();
            clsPacket.FTDI_API_Close();
            //End by FJ for the model name display error in updateFW and updateOEM tool in USB connection, 2016-06-16
            ...
            do
            {
                ...
                //Mod by Wayne for remove incorrect detect code, 2015-01-14
                /*
                //Mod by Wayne for detect and set correct baud rate, 2014-08-15
                //open Serial
                bool errflag = false;
                uint errcount = 0;
                ...
                for (int i = 0; i < DetectBaudRate.Length; i++ )
                {
                    ...
                    if (clsPacket.TRANS_API_Serial_Open(uiLibSettingComPort, ref pDcb) == TRANS_RESULT.OK)
                    {
                        ...
                        {
                            errflag = false; //select correct baud rate
                            break;
                        }
                    }
                    else
                ...
                */
                ...
                //Mod by FJ for the model name display error in updateFW and updateOEM tool in USB connection, 2016-06-16
                //clsPacket.TRANS_API_Close();
                //clsPacket.FTDI_API_Close();
                //End by FJ for the model name display error in updateFW and updateOEM tool in USB connection, 2016-06-16
                
                //if (clsPacket.TRANS_API_Serial_Open(uiLibSettingComPort, ref pDcb) != TRANS_RESULT.OK)
                if (clsPacket.Mid_API_Serial_Open(uiLibSettingComPort) != true)
                //End by Wayne for implement high baud rate function, 2015-06-05
                {
                ...
                //End by Wayne for detect and set correct baud rate, 2014-08-15
                //End by Wayne for remove incorrect detect code, 2015-01-14
            }while( false );


            //Set USB/RS232 Overlap time
            //Mod by Wayne for implement high baud rate function, 2015-06-05
            clsPacket.Mid_API_SetOverlapTime(400, 400);
            //clsPacket.TRANS_API_SetOverlapTime( 400, 400 );
            //End by Wayne for implement high baud rate function, 2015-06-05
...
//==============================Redio Module Configuration==============================
        ...
        //******//
        public Result API_ConfigReadRegisterGetTemperature
        (
            [In]          UInt16 r_MacVirtualRegAddr,
            [In, Out] ref Int32 r_Value
        )
        {
            ...
            r_Value = BitConverter.ToInt32(buf, 2);
...
//===========================ISO 18000-6C Tag Access================================ 
        ...
        )
        {
            ...
            strcData.pCriteria[btIndex].action.action         = (rfid.Constants.Action)buf[7]; 
...
//======================================TEST===========================================
        ...
        )
        {
            ...
            buf[6] = r_RandomType; //Add by Wayne for add Tx random data feature in RFTest page, 2014-09-10
        ...
        public Result API_EngTestGetTemperature
        (
            [In] byte rfu,
            [In, Out] ref int r_Value
        )
        {

#if _TRACE_OUT_PUT
            PrintMagToTxt("API_EngTestGetTemperature");
#endif
            ...
            TxCmd = ENUM_CMD.ENG_TEST_GET_TEMPERATURE;
            buf[0] = (byte)TxCmd;
            buf[1] = (byte)rfu;
            ...
            r_Value = BitConverter.ToInt32(buf_temp, 0);
        ...
        public Result API_SetInventoryResponseControl
        (
            [In] ResponseResponseFormat ExtendInfo
        )
        {

#if _TRACE_OUT_PUT
            PrintMagToTxt("API_SetInventoryResponseControl");
#endif
            Result result = Result.OK;
            ENUM_CMD TxCmd = ENUM_CMD.NOTHING;
            byte[] buf = new byte[10];
            ...
            TxCmd = ENUM_CMD.l8K6C_SET_INVENTORY_RESPONSE_CONTROL;
            buf[0] = (byte)TxCmd;
            buf[1] = (byte)ExtendInfo;
        ...
        public Result API_GetInventoryResponseControl
        (
            [In, Out] ref ResponseResponseFormat r_packet_format
        )
        {

#if _TRACE_OUT_PUT
            PrintMagToTxt("API_GetInventoryResponseControl");
#endif
            Result result = Result.OK;
            ENUM_CMD TxCmd = ENUM_CMD.NOTHING;
            byte[] buf = new byte[10];
            ...
            TxCmd = ENUM_CMD.l8K6C_GET_INVENTORY_RESPONSE_CONTROL;
            buf[0] = (byte)TxCmd;
            ...
            r_packet_format = (ResponseResponseFormat)buf[2];

            return Result.OK;
        }

		// Add by Chingsheng for Command Test Window, 2015-01-30
        public ENUM_CMD Command_Test
        (
            [In] byte[] in_Buffer,
            [In, Out] ref byte[] out_Value
        )
        {

#if _TRACE_OUT_PUT
            PrintMagToTxt("Command_Test");
#endif

            ENUM_CMD TxCmd = ENUM_CMD.COMMAND_TEST;
            ENUM_CMD result = ENUM_CMD.NOTHING;

            if (false == CmdTestSendData(in_Buffer))
                return result;

            Array.Clear(out_Value, 0, out_Value.Length);

            
            result = CmdTestReceiveData(TxCmd, ref out_Value);
...
//===========================ISO 18000-6C Tag-Protocol Custom Operation============================
        public Result API_l8K6CTagNXPSpecific
        (
            [In] NXPSpecificFunID r_functionID,
            [In] NXPChangeConfig r_NXPConfigParms,
            [In] NXPTAM1Authenticate r_NXPTAM1Parms,
            [In] NXPTAM2Authenticate r_NXPTAM2Parms,
            [In] ReadParms r_Parms,
            [In] byte r_PerformFlags
        )
            {
...
#if _TRACE_OUT_PUT
            PrintMagToTxt("API_l8K6CTagNXPSpecific");
#endif
            ...
            TxCmd = ENUM_CMD.l8K6C_TAG_NXP_SPECIFIC;
            buf[0] = (byte)TxCmd;

            //push dtat to buffer
            switch (r_functionID)
            {
                case NXPSpecificFunID.ConfigParms:
                    buf[1] = (byte)NXPSpecificFunID.ConfigParms;
                    buf[2] = (byte)(r_NXPConfigParms.configWord & 0xFF);
                    buf[3] = (byte)((r_NXPConfigParms.configWord >> 8) & 0xFF);
                    buf[4] = 0;
                    buf[5] = 0;
                    buf[6] = 0;
                    buf[7] = r_NXPConfigParms.retryCount;
                    buf[8] = r_NXPConfigParms.performOption;

                    break;

                case NXPSpecificFunID.TAM1Authenticate:
                    buf[1] = (byte)NXPSpecificFunID.TAM1Authenticate;
                    buf[2] = (byte)r_NXPTAM1Parms.replySetting;
                    buf[3] = (byte)r_NXPTAM1Parms.keyID;
                    buf[4] = (byte)r_NXPTAM1Parms.retryCount;
                    buf[5] = (byte)r_NXPTAM1Parms.performOption;

                    break;

                case NXPSpecificFunID.TAM2Authenticate:
                    buf[1] = (byte)NXPSpecificFunID.TAM2Authenticate;
                    buf[2] = (byte)r_NXPTAM2Parms.replySetting;
                    buf[3] = (byte)r_NXPTAM2Parms.memoryProfile;
                    buf[4] = (byte)(r_NXPTAM2Parms.offset & 0xFF);
                    buf[5] = (byte)((r_NXPTAM2Parms.offset >> 8) & 0xFF);
                    buf[6] = (byte)r_NXPTAM2Parms.blockCount;
                    buf[7] = (byte)r_NXPTAM2Parms.retryCount;
                    buf[8] = (byte)r_NXPTAM2Parms.performOption;
...
//==========================================================================
        ...
        )
        {
            ...
            if (clsPacket.Mid_API_Write(wtBuf, (UInt32)wtBuf.Length) != true)
            {
		...
		// Add by Chingsheng for Command Test Window, 2015-01-30
        //============================== Command Test Start =================================
        protected bool CmdTestSendData
        (
            byte[] btData
        )
        {
        ...
        protected ENUM_CMD CmdTestReceiveData
        (
            [In]          ENUM_CMD r_TxCmd,
            [In, Out] ref byte[]   r_btData
        )
        {
            ...
            ENUM_CMD result = ENUM_CMD.NOTHING;
            byte[] rdBuf = new byte[r_btData.Length];
            Array.Clear(rdBuf, 0, rdBuf.Length);
            ...
            while (result != r_TxCmd)
            {
                result = CmdTestRegular(ref rdBuf, true);

#if _TRACE_OUT_PUT
                PrintMagToTxt("CmdTestReceiveData");
#endif
        ...
        public static ushort CrcCul // Mod by Chingsheng for Command Test Window, 2015-01-30
        (
            [In, Out] byte[] buf,
            [In]      uint   uiOffset,
        ...
        protected void ResetRxFlow(ENMU_PKT_TYPE r_Type)
        {
            ...
            if (RxPktType == ENMU_PKT_TYPE.TAG_ACCESS)
            {
                bCheckTimeOut        = false;
        ...
        )
        {
            ...
            switch (MTIresult)
            {
                ...
                case (byte)Global.ENUM_CMD_RESULT.FWUPD_STAT_EXIT_MN_ERR:
                    result = Result.FWUPD_STAT_EXIT_MN_ERR;
                    break;

                case (byte)Global.ENUM_CMD_RESULT.FWUPD_STAT_EXIT_UPDKEY_ERR:
                    result = Result.FWUPD_STAT_EXIT_UPDKEY_ERR;
                    break;
            ...
            try
            {
                while (true) 
                {
                    ...
                    switch (result)
                    {
                        ...
                        case ENUM_CMD.UPDATE_ENTER_UPDATE_MODE:
            #if _TRACE_OUT_PUT
                            PrintMagToTxt("CheckTagAccessPkt: UPDATE_ENTER_UPDATE_MODE");
            #endif    
                        ...
                        case ENUM_CMD.TAG_ACCESS:
                            ...
                            strcParms.tagFlag = true; //Add by FJ for NXP authentication function, 2018-02-02
                            
                            //If get first packet, reset the flow.
                            if (usDivideSeq == 1)
                            {
                                //Add by FJ for NXP authentication function, 2018-02-02
                                if (SingleBuf[0] == 73) //73 = 0x49 = MTII
                                {
                                    Array.Copy(SingleBuf, strcParms.bufMTII, SingleBuf.Length);
                                }
                                if (SingleBuf[0] == 65) //65 = 0x41 = MTIA
                                {
                                    Array.Copy(SingleBuf, strcParms.bufMTIA, SingleBuf.Length);
                                }
                                //End by FJ for NXP authentication function, 2018-02-02
                                usDataTotalLenDw = usDataSingleLenDw;
                            ...
                            if ( true == CheckPktEnd(SingleBuf) )   //report end
                            {
                                ...
                                else    //Multi packet
                                {
                                    ...
                                    if (SingleBuf[0] == 65) //65 = 0x41 = MTIA
                                    {
                                        Array.Copy(SingleBuf, strcParms.bufSecondeMTIA, SingleBuf.Length);
                                    }
                                    //End by FJ for NXP authentication function, 2018-02-02
                                    //combine data
...
//                                    MultiBuf.RemoveRange(MultiBuf.Count - 3, 2);
                    ...
                    TimeBeginPeriod(1);
                    //Mod by Wayne for implement high baud rate function, 2015-06-05
                    if (result != ENUM_CMD.UPDATE_ENTER_UPDATE_MODE)
                    {
                        System.Threading.Thread.Sleep((int)Settings.Default.uiShortInterval);
                    }
                    else
                    {
        ...
        protected ENMU_PKT_TYPE  RxPktType = ENMU_PKT_TYPE.NO;
        ...
        )
        {
            ENUM_CMD         result = ENUM_CMD.NOTHING;
            TRANS_DEV_TYPE   enumDev = clsPacket.TRANS_API_AskDevType();
            byte[] tmpBuf = new Byte[64]; //Add by Wayne for test
...
#endif    
            //Mod by Wayne for implement high baud rate function, 2015-06-05
            UInt32 RevCount = clsPacket.Mid_API_AskRevCount();
            ...
            if (enumDev == TRANS_DEV_TYPE.USB)
            {
				...
				if ( 0 == clsPacket.Mid_API_Read(RxBuf, RevCount) )
                    return result;
			//End by Wayne for implement high baud rate function, 2015-06-05
            ...
            for (int i = 0; i < RevCount; i++)
            {
                switch (RxStep)
                {
                    case ENMU_PKT_FLOW.STEP_HEADER_1:
                        ...
                        clsPacket.Mid_API_Read(RxByte, 1);
                        //End by Wayne for implement high baud rate function, 2015-06-05
		...
		// Add by Chingsheng for Command Test Window, 2015-01-30
        protected ENUM_CMD CmdTestRegular
        (
            [In, Out] ref byte[] RxBuf,
            [In]          bool bCheckTimeOut
        )
        {
            ...
            //End by Wayne for implement high baud rate function, 2015-06-05
                ...
                if (CmdTestPutPktToData(enumDev, RxBuf) == ENMU_REV_STATE.REVEICE_DONE)
                {
                    result = RxCmd;
                }
        ...
        )
        {
            ...
            try
            {
                ...
                switch (RxBuf[0])
                {
                    ...
                    case (byte)'B': //Begin
                    case (byte)'E': //End
                        RxPktType = ENMU_PKT_TYPE.TAG_ACCESS;
                        usPktSingleLenBt = 24;
                    ...
                    //Common==========================
                    case (byte)'R': //Common
                        RxPktType = ENMU_PKT_TYPE.COMMON;
                        usPktSingleLenBt = 16;
                    ...
                    case (byte)'F': //Update firmware mode
                        RxPktType = ENMU_PKT_TYPE.UPDATE_ENTER;
                        usPktSingleLenBt = 24;
                        Length = usPktSingleLenBt - (int)ENMU_TAG_PKT_INDEX.PKT_DEVIDE_NUM;
                        break;
                    ...
                    case (byte)'S': //Update Common
                        RxPktType = ENMU_PKT_TYPE.UPDATE_COMMON;
                        usPktSingleLenBt = 64;
                ...
                if (enumDevice == TRANS_DEV_TYPE.SERIAL)
                {
                    ...
                    if (clsPacket.Mid_API_AskRevCount() < Length)
                    {
                    ...
                    if (Length != clsPacket.Mid_API_Read(tmp, (uint)Length))
                    //End by Wayne for implement high baud rate function, 2015-06-05
                    {
                        RxStep = ENMU_PKT_FLOW.STEP_HEADER_1;
                        return ENMU_REV_STATE.RECEIVE_FAULT;
                    ...
                    case ENMU_PKT_TYPE.UPDATE_ENTER:
                        {
                            ...
                            RxCmd   = ENUM_CMD.UPDATE_ENTER_UPDATE_MODE;
		...
		// Add by Chingsheng for Command Test Window, 2015-01-30
        protected ENMU_REV_STATE CmdTestPutPktToData
        (
            [In] TRANS_DEV_TYPE enumDevice,
            [In] byte[] RxBuf
        )
        {
...
#if _TRACE_OUT_PUT
            PrintMagToTxt("CmdTestPutPktToData");
#endif
            ...
            {
#if _TRACE_OUT_PUT
                this.PrintMagToTxt("Exception Error : CmdTestPutPktToData");
#endif
                //clsPacket.TRANS_API_ClearBuffer();
                clsPacket.Mid_API_ClearBuffer();
                //End by Wayne for implement high baud rate function, 2015-06-05
